...
namespace DVZ {
	...
	public:
		...
		using value_type = glm::vec<3, IndexType>;
		...
		SpiralIter(value_type center, IndexType radius) {
			if (radius >= 0) {
				this->center = center;
				this->current_coord = value_type{0};
				this->radius = radius;
				this->layer = 1;
				this->leg = 0;
			}
		...
		value_type operator*() const { return (center + current_coord); }
		pointer operator->() const { return &(center + current_coord); }

		// Prefix increment
		SpiralIter& operator++() {
			assert((*this) != SpiralIter{});
			
			if (current_coord.x == radius && current_coord.z == -radius) {
				(*this) = SpiralIter{};
			}
			else {
				auto& x = current_coord.x;
				auto& z = current_coord.z;
				switch (leg) {
				case 0:
					++x; if (x == layer) ++leg; break;
				case 1:
					++z; if (z == layer) ++leg; break;
				case 2:
					--x; if (-x == layer) ++leg; break;
				case 3:
					--z; if (-z == layer) { leg = 0; ++layer; } break;
				}
			}
	...
	private:
		value_type center = value_type{0};
